...
import org.jivesoftware.smack.initializer.VmArgInitializer;
...
public class XmppChatClass {
	...
	public void performLogin(String username, String password,
		...
		try {
			...
			//handleOfflineMessages(xmppConnection);
			...
			xmppConnection.addPacketListener(new PacketListener() {
	            ...
	            public void processPacket(Packet p) {
	            	...
	            	Message  message = (Message) p;// this was the problem

	                if (message.getBody() != null) {
	                    System.out.println("Reciveing text [" + message.getBody() + "] ");
	                    (new ReverseClass()).listeningForMessages(message);
	                }
	            }
	...
	/*public  void handleOfflineMessages(XMPPConnection connection)throws Exception {
        ...
        if (!offlineMessageManager.supportsFlexibleRetrieval()) {
            System.out.println("Offline messages not supported");
            return;
        }

        if (offlineMessageManager.getMessageCount() == 0) {
        	System.out.println("No offline messages found on server");
        } else {
            ...
            for (Message msg : msgs) {
               ...
               // String bareJid = StringUtils. parseBareAddress(fullJid);
                ...
                if (messageBody != null) {
                	System.out.println("Retrieved offline message from "+fullJid+"--- " +messageBody);
                }
            }
	...
	public void clearRoster() {
		System.out.println("in clearRoster");
		Roster roster = xmppConnection.getRoster();
		...
		Presence presence = new Presence(Presence.Type.unsubscribe);
		presence.setTo(re.getUser());
		try {
			xmppConnection.sendPacket(presence);
		} catch (NotConnectedException e) {
			e.printStackTrace();
		}
		System.out.println("unsubscribe sent to in deny friend request %%%%%%%%%%%%% "+ re.getUser());
		System.out.println("entry to be deleted "+re.getUser());
		try {
