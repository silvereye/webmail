...
<%
String chatService=(String)head_hs.getAttribute("chatService");
	if(chatService.equalsIgnoreCase("true"))
	{
%>
	setTimeout(function(){ getChatBody(); }, 2000);
	<%
	}
	%>
	...
	setTimeout(function(){ showcalNoti(); }, 12000);
		...
		$(document.body).on('keydown','.search_clear',function(e){
      	//   alert(e.which);
      	   if((e.which == 13 ))
      		   {
      			 mailAdvSearch(e);
...
<%!
public List<MailImapFolders> lFilesnp(List<MailImapFolders> list, String path, String host, String id, String pass, WebmailClient webmailClient) {
    	        	...
    	        	if(fd.isHasChild())
    	   			{
    	   				...
    	   				lFilesnp(list, fd.getFolderFullName(),host, id, pass,  webmailClient);
    	   			}
    	   			else
    	   			{
...
%>
    ...
    dom=arr[arr.length-1];
				...
				List<String> all_fldr_lst_search=new ArrayList();
            %>


				<%

		  		  for(MailImapFolders fd : imapfldrlst_search)
		  			{
		  			...
		  			if(fd.isHasChild())
		  			{
		  				List<MailImapFolders> list1=lFilesnp(list, fd.getFolderFullName(), host, id, pass, webmailClient);
		  				...
		  				for(int i=0;i<list1.size();i++)
		  				{
		  					...
		  					all_fldr_lst_search.add(list1.get(i).getFolderFullName());
		  			...
		  			{
		  					all_fldr_lst_search.add(fd.getFolderFullName());
		...
		%>
						...
						<!---/// TAG CONTENT STRED HERE ------->
                            <div class="tag_content">
                                 <div class="tag_inner_content">
                                       <ul>
                                         <!-- <li><input type="checkbox" name="my_chk_tag" value="$label1" class="tag_check"><span><div class="append_color" style="background: #FA6855;"></div><div class="append_val">Important</div></span><div style="background: #FA6855;" class="tag_option_fix"><img src="images/cal-open.png"></div></li> -->
                                           <li><input type="checkbox" name="my_chk_tag" value="$label1" class="tag_check"><span><div class="append_color" style="background: #FA6855;"></div><div class="append_val">Important</div></span><div style="background: #FA6855;" class="tag_option_fix"></div></li>
                                           <li><input type="checkbox" name="my_chk_tag" value="$label2" class="tag_check"><span><div class="append_color" style="background: #FC9449;"></div><div class="append_val">Work</div></span><div style="background: #FC9449;" class="tag_option_fix"></div></li>
                                           <li><input type="checkbox" name="my_chk_tag" value="$label3" class="tag_check"><span><div class="append_color" style="background: #98C95D;"></div><div class="append_val">Personal</div></span><div style="background: #98C95D;" class="tag_option_fix"></div></li>
                                           <li><input type="checkbox" name="my_chk_tag" value="$label4" class="tag_check"><span><div class="append_color" style="background: #486BF7;"></div><div class="append_val">To Do</div></span><div style="background: #486BF7;" class="tag_option_fix"></div></li>
                                            <li><input type="checkbox" name="my_chk_tag" value="$label5" class="tag_check"><span><div class="append_color" style="background: #BD48F7;"></div><div class="append_val">Later</div></span><div style="background: #BD48F7;" class="tag_option_fix"></div></li>
                                        ...
                                        <%
                                         ...
                                         {
                                        	// System.out.println("in setting--"+labset);
                                        	 String tagarr[]=labset.split("~");
                                        	//System.out.println("in setting len--"+tagarr.length);
                                        	 for(int i=0;i< tagarr.length; i++)
                                        	 {
                                        	//	 System.out.println("in setting "+i+"th--"+tagarr[i]);
                                        		 String tarr[]=tagarr[i].split("#");
                                        	//	 System.out.println("in setting "+tarr[0]+"-----"+tarr[1]);
                                        	 String tgnm=tarr[0];
                                        	 tgnm=tgnm.replace("_", " ");
                                        	// System.out.println("in setting "+tgnm);
                                  		%>
                                          <li><input type="checkbox" name="my_chk_tag" value="<%=tgnm %>" class="tag_check"><span><div class="append_color" style="background: #<%=tarr[1] %>;"></div><div class="append_val"><%=tgnm %></div></span><div style="background: #<%=tarr[1] %>;" class="tag_option_fix"></div></li>
                            <%} 
                            }%>
                                 ...
                                 <ul>
                                    <!--  <li class="craet_gat">Create New Label  <div class="clear"></div></li> -->
                                     <li onclick="addTag()" class="apply_tag"> Apply   <div class="clear"></div></li>
                                 </ul>
                                 <div class="clear"></div>
						  ...
						  <!-------/// FOLDER CONTENT STARED HERE ------->
                            <div class="folder_div">
                                   <ul>
                                   
                                   <%
                     for(String val_fdr: all_fldr_lst_search)
                     {
    					...
    					String fnm_tmp=val_arr[val_arr.length-1];
    					 if(fnm_tmp.length()>25)
    					{
    						fnm_tmp=fnm_tmp.substring(0, 24)+"....";
    					} */
    					 if(!val_fdr.equalsIgnoreCase("Drafts"))
    					 {
                    	 %>
                      <%--    <option title="<%=val_arr[val_arr.length-1] %>" value="<%=val_fdr %>"> <%=sps+fnm_tmp %></option> --%>
                             <li onclick="moveToFolderAll(this.id)" id="<%=val_fdr %>"><a  title="<%=val_fdr %>" ><%=val_fdr %></a></li>
                         <%
    					 }
                     }
                     %>
...
<br>
<p id="sec_info" style="text-align: center; font-size: 14px; color: #616161; margin-top: 25px;"></p>
</div>
	...
	<div class="minwidth">
		...
		<div class="header">
			<div class="header_inner">
				<div class="logo">
					<a style="cursor: pointer;" onclick="refreshInbox()"> <img class="head_logo" src="logo/<%=dom %>.png" onerror="getHeadLogo(this.id)" id="main_logo"/>
					</a>
				</div>
					...
					<div class="main_search_folder">
						<div class="search_it">
							<input type="text" id="quick_search" name="quick_search"
								onClick="return Hide();" onkeypress="keyPresSearch(event)" placeholder="Search Here...." />
							<a style="cursor: pointer;">
								<div class="down_arrow" title="Show search options" onClick="return showHide(event);"></div>
							</a>
						...
						<ul>
							...
							<li><a href="contacts" target="_blank">
									<div class="contact_ring">
							         ...
							         <li>
                            <a style="cursor: pointer;" href="tasks" target="_blank">
...
<%--  <div style="color:#fff;    float: right;"><%=id %></div>   --%>
		...
		<div class="content">
                     ...
                     %>
                      ...
                      -->
                          ...
                          <option value="$label5"> Later</option>
                           ...
                           if(labset!=null && labset.length()>0)
							...
							<div style="padding-top: 15px;"></div>
								...
								<div class="name search_text">Status</div>
							<a><div>
									<select id="webmail_status" name="webmail_status"
										style="width: 227px; height: 32px; border: 1px solid #ccc !important; padding-left: 3px;">
 										<option value=""> Select Status</option>
                          <option value="Read"> Read</option>
                          <option value="Unread"> Unread</option>
                          <option value="Stared"> Starred</option>
                          <option value="Unstared"> Unstarred</option>
							...
							<input type="text" placeholder="To" name="to_search" value=""
								class="search_clear border input" id="to_search">
							<div style="padding-top: 50px;"></div>
							<input type="text" placeholder="From" name="from_search" value=""
								class="search_clear border input" id="from_search">
							<div style="padding-top: 50px;"></div>
							<input type="text" name="sub_search" placeholder="Subject"
								value="" class="search_clear border input" id="sub_search">
							<div style="padding-top: 50px;"></div>
							<input type="text" name="con_search" placeholder="Content"
								value="" class="search_clear border input" id="con_search">
							<div style="padding-top: 50px;"></div>
							<table>
							<tr>
							<td style="    padding-bottom: 10px;"> To</td><td><input type="text" name="startdate" placeholder="yyyy-MM-dd" value=""
								class="popupDatepicker search_clear" id="startdate"  style=" width: 111px !important;    height: 30px !important;"> </td>
							<td style="    padding-bottom: 10px;">From</td><td><input type="text" name="enddate" placeholder="yyyy-MM-dd" value=""
								class="popupDatepicker search_clear" id="enddate" style=" width: 111px !important;    height: 30px !important;">  </td>
							</tr>
							</table>
						...
						<div class="search_button">
							<a onclick="mailAdvSearch(this)" style="cursor: pointer;"
								title="Search"> <span class="search_icon2"> </span></a>
						</div>
					</form>

					<input type="hidden" value="" id="hid_quick_search" /> <input
						type="hidden" value="" id="hid_to_search" /> <input type="hidden"
						value="" id="hid_from_search" /> <input type="hidden" value=""
						id="hid_sub_search" /> <input type="hidden" value=""
						id="hid_con_search" /> <input type="hidden" value=""
						id="hid_date1_search" />
			...
			<div class="user_information arrow_box">
				...
				<div class="user_top">
					<a id="big_img" style="cursor: pointer;"> <img height='96px' width='96px'
						alt="Embedded Image" src="<%=path_img %>" onerror="getAltImage(this.id)"
						id="big_<%=path_img_id %>" />
					...
					<div class="left_top">
						<a name="userName"> <%=fname %></a> <span><%=mailid %></span>
				...
				<div class="user_bottom">
					...
					</a> <a onclick="openSettingProfile()" style="cursor: pointer;">
